...
def lambda_handler(event, context):
    ...
    bot_token = ssm.get_parameter(
        Name=f'{env}-discord-fighter-bot-token', WithDecryption=True)['Parameter']['Value']
    ...
    async def role(message):
        ...
        # split string at role and get what user typed after. covers `.getrole` and `.role`
    ...
    async def match(message):
        ...
        for m in queue:  # for each message in the queue
            # timestamp is stored as an expiration time in epoch format, so if the current time is equal to or greater than that time,
            if time.time() >= float(m['Time']['N']):
                ddb.delete_item(TableName=table_name, Key={
                                'Author': m['Author']})  # expire the message
        # default false, will change to true to avoid extra messages when player refrehses
        ...
        if 'cancel' in message.content:
            ...
            try:
                # removes whitespace, takes number that should be after `.mm`, rounds it and converts to int
            ...
            format_message['timestamp'] = format_message['timestamp'] + \
                (mins * 60)
            # regardless of whether the author had an old message removed from queue or not, add their new message to the end
            ddb.put_item(TableName=table_name, Item={'Author': {'S': str(
                format_message['author'])}, 'Time': {'N': str(format_message['timestamp'])}})
            ...
            if len(queue) > 1:
                ...
                # clear those authors
                ddb.delete_item(TableName=table_name, Key={
                                'Author': {'S': queue[0]['Author']['S']}})
                ddb.delete_item(TableName=table_name, Key={
                                'Author': {'S': queue[1]['Author']['S']}})
