...
def lambda_handler(event, context):
    ...
    bot_token = ssm.get_parameter(
        Name=f"{env}-discord-fighter-bot-token", WithDecryption=True
    )["Parameter"]["Value"]
    table_name = os.environ["TableName"]

    # bot commands. reminder to avoid clashing with other bots - multiple
    ...
    async def role(message):
        # immediately look for role string, split string at role and get what
        # user typed after. covers `.getrole` and `.role`
        role_string = message.content.split("role ")[1]
        if len(role_string) < 3:  # discard invalid role strings
            await message.channel.send(
                content="Not recognised as an available role; please retry. "
            )
            return
        ...
        for char in chars:
            for term in char["Terms"]:
                if (
                    term.lower() in role_string.lower()
                ):  # add lower to both to prevent case problems
        ...
        role = discord.utils.get(
            message.author.guild.roles, name=selected_role_name
        )
        if (
            # should only trigger if user doesn't have role
            selected_role_name not in [role.name for role in message.author.roles]
        ):
            await message.author.add_roles(role)
            await message.channel.send(
                content=f"Added role {selected_role_name} for {message.author}. "
            )
        else:
            await message.author.remove_roles(role)
            await message.channel.send(
                content=f"Removed role {selected_role_name} from {message.author}. "
            )
    ...
    async def match(message):
        ...
        for m in queue:  # for each message in the queue
            # timestamp is stored as an expiration time in epoch format, so if
            # the current time is equal to or greater than that time,
            if time.time() >= float(m["Time"]["N"]):
                ddb.delete_item(
                    TableName=table_name, Key={"Author": m["Author"]}
                )  # expire the message
        # default false, will change to true to avoid extra messages when player
        # refreshes
        player_refreshed = False
        for m in queue:
            # check if author already in queue. if so, remove
            if int(m["Author"]["S"]) == message.author.id:
                print(f"{message.author} is already queued; removing old entry. ")
                player_refreshed = True  # for later use
                ddb.delete_item(
                    TableName=table_name, Key={"Author": {"S": str(message.author.id)}}
                )
        if "cancel" in message.content:
            print(f"{message.author} has requested to cancel; not adding new entry. ")
            await message.channel.send(
                content=f"{message.author} has been removed from the queue. "
            )
        else:
            # format message
            format_message = {}
            ...
            try:
                # removes whitespace, takes number that should be after `.mm`,
                # rounds it and converts to int
                mins = round(float(message.content.split()[1]))
                if mins > 120:
                    mins = 120
                elif mins < 5:
                    mins = 5
            ...
            # regardless of whether the author had an old message removed from
            # queue or not, add their new message to the end
            ddb.put_item(
                TableName=table_name,
                Item={
                    "Author": {"S": str(format_message["author"])},
                    "Time": {"N": str(format_message["timestamp"])},
                },
            )
            ...
            if (
                player_refreshed == False
            ):  # don't do a duplicate message if they were already told they refreshed
                await message.channel.send(
                    content=f"{message.author} has been added to the matchmaking queue for {mins} minutes. "
                )
            else:
                await message.channel.send(
                    f"{message.author} is already in the queue; refreshing timeout ({mins} minutes). "
                )
            ...
            if len(queue) > 1:
                ...
                print(
                    f"{queue[0]['Author']['S']} and {queue[1]['Author']['S']} have been matched. They will be removed from the queue. "
                )
                # clear those authors
                ddb.delete_item(
                    TableName=table_name, Key={"Author": {"S": queue[0]["Author"]["S"]}}
                )
                ddb.delete_item(
                    TableName=table_name, Key={"Author": {"S": queue[1]["Author"]["S"]}}
                )
                # send message to channel that the second challenger sent the '.mm' message in
                await message.channel.send(content=announce)

    # read messages in chat to determine if they have commands for the bot and
    # respond accordingly
    ...
    async def on_message(message):
        ...
        if message.content.startswith(help_commands):
            ...
            await message.channel.send(
                content=mm_help_string + role_help_string + generic_help_string
            )
    ...
    async def on_ready():
        ...
        await client.change_presence(
            activity=discord.Game(name=".help for commands", type=0)
        )
